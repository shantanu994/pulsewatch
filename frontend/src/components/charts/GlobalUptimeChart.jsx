...
export default function GlobalUptimeChart({
  checks,
  hours,
  onHoursChange,
  ...
  const overall =
    windowed.length === 0
      ? null
      : Math.round(
          (windowed.filter((c) => c.is_up).length / windowed.length) * 10000,
        ) / 100;
    ...
    <div className="bg-panel border border-white/5 rounded-xl p-5">
      ...
      {data.length < 2 ? (
        <div className="h-48 flex items-center justify-center text-slate text-sm text-center px-6">
          Not enough data. Analytics will appear once enough checks are
          collected.
        </div>
      ) : (
        <div className="h-52">
